...
class JoshController extends Controller {
	...
	*/
    ...
    public function showHome()
    {
        ...
        if(Sentinel::check())
            return view('admin.index',[ 'analytics_error'=>$analytics_error,'chart_data'=>$chart_data, 'blog_count'=>$blog_count,'geo'=>$geo,'user_count'=>$user_count,'users'=>$users,'db_chart'=>$db_chart,'user_roles'=>$user_roles,'blogs'=>$blogs,'visitors'=>$visitors,'pageVisits'=>$pageVisits,'line_chart'=>$line_chart,'month_visits'=>$month_visits,'year_visits'=>$year_visits] );
        else
            return redirect('admin/signin')->with('error', 'You must be logged in!');
    }
}
